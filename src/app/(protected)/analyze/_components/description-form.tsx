...
export function DescriptionForm({ onSubmit, isLoading }: DescriptionFormProps) {
  ...
  return (
    <div className="w-full relative bg-white/70 backdrop-blur-2xl border border-white p-2 sm:p-3 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] ring-1 ring-slate-900/5">
        ...
        <div className="relative group">
          ...
          <div className="absolute bottom-3 left-3 right-3 flex flex-col sm:flex-row gap-3 sm:justify-between sm:items-center">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-2.5 bg-slate-900 text-white font-bold text-sm rounded-xl shadow-md hover:bg-primary focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all disabled:opacity-50 shrink-0 flex items-center justify-center gap-2 group/submit"
            >
              {isLoading ? (
                <>
                ...
                <>
                  BẮT ĐẦU
                  <div className="bg-white/20 p-1 rounded-md ml-1 group-hover/submit:bg-white/30 transition-colors">
                     <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
