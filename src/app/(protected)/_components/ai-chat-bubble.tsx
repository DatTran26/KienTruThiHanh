    ...
    <div className="fixed bottom-[110px] right-6 z-50 flex flex-col items-end pointer-events-none">
      ...
      {isOpen && (
        <div className="mb-5 w-[400px] h-[600px] pointer-events-auto bg-white/70 backdrop-blur-3xl rounded-[28px] shadow-[0_20px_80px_-15px_rgba(0,0,0,0.2)] border border-white/80 overflow-hidden flex flex-col animate-in slide-in-from-bottom-12 fade-in zoom-in-95 duration-500 origin-bottom-right">
          ...
          <div className="relative px-6 py-5 bg-gradient-to-r from-indigo-50 to-white/90 border-b border-indigo-100 flex items-center justify-between shrink-0 overflow-hidden">
            ...
            <div className="relative z-10 flex items-center gap-3.5">
              ...
              <div className="flex flex-col">
                <h3 className="text-[17px] font-extrabold text-indigo-950 tracking-tight flex items-center gap-2">
                  Aurora AI
                  <div className="px-2 py-0.5 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center">
            ...
            <button 
              ...
              className="relative z-10 size-9 rounded-full bg-indigo-50 hover:bg-indigo-100 flex items-center justify-center text-indigo-400 hover:text-indigo-600 transition-all hover:rotate-90 hover:scale-110 shadow-sm border border-indigo-100/50"
            >
            ...
            {messages.map((msg, idx) => (
              ...
              >
                ...
                <div className={`px-4 py-3.5 text-[14px] leading-relaxed shadow-sm ${
                  msg.role === 'user' 
                    ? 'bg-gradient-to-br from-indigo-500 via-indigo-600 to-indigo-700 text-white rounded-2xl rounded-tr-sm shadow-indigo-500/20 shadow-lg border border-indigo-400/30 font-medium'
                    : 'bg-white/95 backdrop-blur-md border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] text-slate-700 rounded-2xl rounded-tl-sm font-medium'
                }`}>
                  {msg.content}
                </div>
              </div>
          ...
          {/* Advanced Input Area */}
          <div className="px-4 py-5 bg-white backdrop-blur-xl border-t border-slate-100 shadow-[0_-10px_40px_rgba(0,0,0,0.02)] shrink-0">
            <div className="relative flex items-center max-w-full group">
              {/* Outer glow on focus-within */}
              <div className="absolute -inset-1 bg-gradient-to-r from-indigo-300 to-violet-300 rounded-[20px] opacity-0 group-focus-within:opacity-50 blur transition-opacity duration-300"></div>
              ...
              <input
                ...
                className="relative w-full h-[52px] pl-5 pr-14 rounded-[18px] bg-slate-50 border-2 border-slate-100 focus:bg-white text-[14.5px] outline-none focus:border-indigo-400 text-slate-700 placeholder:text-slate-400 font-semibold transition-all shadow-sm"
                disabled={loading}
              />
              <button 
                onClick={handleSend}
                disabled={!input.trim() || loading}
                className="absolute right-2 size-10 rounded-[14px] bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center text-white transition-all shadow-md shadow-indigo-600/20 active:scale-95"
              >
